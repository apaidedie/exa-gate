...
Exa Reverse Proxy 是一个可自托管的 Exa API 控制平面，面向 AI 搜索产品、Agent 工作流和团队内部服务。业务侧只保留一个客户端令牌；Key 池调度、冷却、故障转移、加密存储、日志审计、告警和控制台治理都在代理层统一处理。你得到的是一个可部署、可验证、可交给团队使用的 Exa 网关，而不是散落在脚本里的密钥轮换逻辑。

![Admin Console](docs/assets/admin-console.png)

桌面首屏展示真实本地 demo：代理链路地图、最近活动、Key 池健康、近期趋势和告警建议会一起给出当前运行判断。截图由 `npm run capture:preview` 从本地渲染生成，不是手工拼图。
...
| VPS / 生产部署 | 下载 `docker-compose.yml` 和 `.env.example` 后 `docker compose up -d` | 只监听本机端口的 Docker 服务，适合放到 HTTPS 反向代理后面。 |
...
管理员令牌、浏览器会话和上游隔离在登录前就可见；Caps Lock 提示会在令牌输入时即时出现，不影响提交。

**桌面运维总览**

桌面截图已放在 README 首屏，方便先判断产品质感和信息密度。概览页聚焦代理链路地图、最近活动、Key 池健康、近期趋势和告警建议，共同形成全局运维判断。
