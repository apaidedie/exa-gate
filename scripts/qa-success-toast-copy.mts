...
const outDir = join(process.cwd(), 'output', 'session-162-success-toast');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-success-toast-'));
...
async function capture(name: string, width: number, height = 844) {
  ...
  // Inject success toast samples for visual QA (export needs download path)
  const text = await page.evaluate(() => {
    const toast = document.getElementById('toast');
    if (!toast) return '';
    const message = '批量操作完成：1 个密钥。可继续筛选状态或打开详情复核。';
  ...
  await page.screenshot({ path: join(outDir, `${name}-batch-success.png`), fullPage: false });
  console.log(name, JSON.stringify({ text, hasNext: text.includes('筛选') || text.includes('详情') }));
  await page.close();
  return text;
}
...
if (!desktop.includes('复核') || !mobile.includes('复核')) {
  console.error('FAIL: success next-step missing', { desktop, mobile });
  process.exit(1);
}
