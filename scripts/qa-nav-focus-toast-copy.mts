...
const outDir = join(process.cwd(), 'output', 'session-163-nav-focus-toast');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-nav-focus-toast-'));
...
async function capture(name: string, width: number, height = 844) {
  ...
  await page.screenshot({ path: join(outDir, `${name}-keys-open.png`), fullPage: false });
  console.log(name, JSON.stringify({ text, hasNext: text.includes('搜索') || text.includes('筛选') }));
  await page.close();
  return text;
}
...
if (!desktop.includes('可搜索') || !mobile.includes('可搜索')) {
  console.error('FAIL: nav focus next-step missing', { desktop, mobile });
  process.exit(1);
}
