...
const outDir = join(process.cwd(), 'output', 'session-136-sort-btn');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-sort-btn-'));
...
async function capture(name: string, width: number, height = 844) {
  ...
  await page.waitForSelector('.keys-panel .sort-btn[data-sort="requests"]');
  await page.evaluate(() => {
    const btn = document.querySelector('.keys-panel .sort-btn[data-sort="requests"]') as HTMLElement | null;
    btn?.scrollIntoView({ block: 'nearest', inline: 'nearest' });
  });
  await page.waitForTimeout(80);

  const metrics = await page.evaluate(() => {
    const sorts = ['requests', 'success', 'failures', 'rateLimits', 'timeouts'];
    const buttons = sorts.map((sort) => {
      const btn = document.querySelector(`.keys-panel .sort-btn[data-sort="${sort}"]`) as HTMLElement | null;
      const rect = btn?.getBoundingClientRect();
      return { sort, height: rect?.height || 0, width: rect?.width || 0 };
    });
    return {
      buttons,
      minHeight: Math.min(...buttons.map((b) => b.height || 999)),
      overflowX: document.documentElement.scrollWidth > document.documentElement.clientWidth
    };
  });

  await page.screenshot({ path: join(outDir, `${name}-sort.png`), fullPage: false });
...
if (Math.round(mobile.minHeight) < 44 || Math.round(tablet.minHeight) < 44) {
  console.error('FAIL: sort buttons below 44px', { mobile: mobile.minHeight, tablet: tablet.minHeight });
  process.exit(1);
}
if (desktop.minHeight > 36) {
  console.error('FAIL: desktop sort buttons unexpectedly tall', desktop.minHeight);
  process.exit(1);
}
