...
const outDir = join(process.cwd(), 'output', 'session-146-key-workflow');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-key-workflow-'));
...
async function capture(name: string, width: number, height = 844) {
  ...
  await page.waitForSelector('#keyWorkflowSummary');
  await page.waitForTimeout(280);
  const reset = await page.locator('[data-key-workflow-action="reset"]').boundingBox();
  const scope = await page.locator('[data-key-workflow-action="scope"]').boundingBox();
  await page.locator('#keyWorkflowSummary').scrollIntoViewIfNeeded();
  await page.screenshot({ path: join(outDir, `${name}-workflow.png`), fullPage: false });
  const metrics = {
    resetH: reset?.height || 0,
    scopeH: scope?.height || 0
  };
...
if (Math.round(mobile.resetH) < 44 || Math.round(tablet.resetH) < 44 || Math.round(mobile.scopeH) < 44) {
  console.error('FAIL: mobile/tablet key-workflow below 44', { mobile, tablet });
  process.exit(1);
}
if (desktop.resetH < 50) {
  console.error('FAIL: desktop key-workflow unexpectedly short', desktop.resetH);
  process.exit(1);
}
console.log('QA screenshots written to', outDir);
console.log('desktop', desktop.resetH, 'mobile', mobile.resetH, 'tablet', tablet.resetH);
