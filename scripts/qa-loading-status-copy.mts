...
const outDir = join(process.cwd(), 'output', 'session-164-loading-status');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-loading-status-'));
...
async function capture(name: string, width: number, height = 844) {
  ...
  // Force visible syncing state for screenshot QA
  const text = await page.evaluate(() => {
    const target = document.getElementById('lastUpdated');
    if (!target) return '';
    target.setAttribute('data-refresh-state', 'syncing');
  ...
  await page.screenshot({ path: join(outDir, `${name}-syncing.png`), fullPage: false });
  console.log(name, JSON.stringify({ text, ok: text.includes('正在同步') }));
  await page.close();
  return text;
}
...
if (!desktop.includes('正在同步') || !mobile.includes('正在同步')) {
  console.error('FAIL: loading status copy missing', { desktop, mobile });
  process.exit(1);
}
