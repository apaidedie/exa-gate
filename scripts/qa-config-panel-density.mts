...
const outDir = join(process.cwd(), 'output', 'qa-config-panel-density');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-config-'));
...
async function capture(name: string, width: number) {
  ...
  await page.waitForSelector('.config-body');
  await page.locator('.config-panel').scrollIntoViewIfNeeded();
  await page.screenshot({ path: join(outDir, `${name}.png`), fullPage: false });
  const metrics = await page.evaluate(() => {
    const body = document.querySelector('.config-body') as HTMLElement | null;
    if (!body) return null;
    const style = getComputedStyle(body);
    const items = Array.from(body.querySelectorAll('.config-item')).map((item) => {
...
await capture('desktop-1440', 1440);
