...
const outDir = join(process.cwd(), 'output', 'session-158-login-error-copy');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-login-error-'));
...
async function capture(name: string, width: number, height = 844) {
  ...
  await page.fill('#loginToken', 'wrong_token_xyz');
  await page.click('#loginButton');
  await page.waitForSelector('#loginError:not([hidden])');
  await page.waitForTimeout(200);
  const text = (await page.locator('#loginError').textContent()) || '';
  await page.screenshot({ path: join(outDir, `${name}-invalid.png`), fullPage: false });

  await page.fill('#loginToken', '');
  await page.click('#loginButton');
  await page.waitForTimeout(200);
  const emptyText = (await page.locator('#loginError').textContent()) || '';
  await page.screenshot({ path: join(outDir, `${name}-empty.png`), fullPage: false });

  console.log(name, JSON.stringify({ invalid: text, empty: emptyText }));
  await page.close();
...
if (!desktop.text.includes('无效') || !desktop.text.includes('重试')) {
  console.error('FAIL: invalid recovery copy missing', desktop.text);
  process.exit(1);
}
if (!desktop.emptyText.includes('请输入') || !desktop.emptyText.includes('demo')) {
  console.error('FAIL: empty recovery copy missing', desktop.emptyText);
  process.exit(1);
}
