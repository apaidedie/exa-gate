...
const outDir = join(process.cwd(), 'output', 'qa-toast-batch-clearance');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-toast-'));
...
async function capture(name: string, width: number) {
  ...
  await expectVisible(page, '#batchBar');
  // Before: toast at default bottom while bar open (force show via evaluate for baseline)
  await page.evaluate(() => {
    const toast = document.getElementById('toast');
    if (!toast) return;
    toast.className = 'toast good';
    toast.textContent = '批量操作完成：1 个密钥（截图基线）';
    toast.style.display = 'block';
    document.documentElement.style.setProperty('--toast-lift', '0px');
    toast.removeAttribute('data-toast-lift');
  });
  await page.screenshot({ path: join(outDir, `${name}-before.png`), fullPage: false });
  // After: apply product lift path
  await page.evaluate(() => {
    document.documentElement.style.removeProperty('--toast-lift');
  });
  await page.click('#batchEnableSelected');
  await expectVisible(page, '#toast');
  await page.waitForTimeout(120);
  await page.screenshot({ path: join(outDir, `${name}-after.png`), fullPage: false });
  await page.close();
}
