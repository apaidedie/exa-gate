...
const outDir = join(process.cwd(), 'output', 'session-144-insight-action');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-insight-action-'));
...
async function capture(name: string, width: number, height = 844) {
  ...
  await page.waitForSelector('#insightNextActionButton');
  await page.waitForTimeout(280);
  const box = await page.locator('#insightNextActionButton').boundingBox();
  await page.locator('#insightNextAction').scrollIntoViewIfNeeded();
  await page.screenshot({ path: join(outDir, `${name}-insight.png`), fullPage: false });
  const metrics = {
    h: box?.height || 0,
    w: box?.width || 0,
    y: box?.y || 0
...
if (Math.round(mobile.h) < 44 || Math.round(tablet.h) < 44) {
  console.error('FAIL: mobile/tablet insight action below 44', { mobile, tablet });
  process.exit(1);
}
if (desktop.h > 40) {
  console.error('FAIL: desktop insight action tall', desktop.h);
  process.exit(1);
}
console.log('QA screenshots written to', outDir);
console.log('desktop', desktop.h, 'mobile', mobile.h, 'tablet', tablet.h);
