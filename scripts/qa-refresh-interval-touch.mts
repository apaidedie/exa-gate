...
const outDir = join(process.cwd(), 'output', 'session-147-refresh-interval');
...
const stateDir = mkdtempSync(join(tmpdir(), 'exa-qa-refresh-interval-'));
...
async function capture(name: string, width: number, height = 844) {
  ...
  await page.waitForSelector('#refreshInterval');
  ...
  await page.locator('.refresh-group').scrollIntoViewIfNeeded();
  await page.screenshot({ path: join(outDir, `${name}-refresh.png`), fullPage: false });
  const metrics = {
    intervalH: interval?.height || 0,
    toggleH: toggle?.height || 0
  };
...
if (Math.round(mobile.intervalH) < 44 || Math.round(mobile.toggleH) < 44 || Math.round(tablet.intervalH) < 44) {
  console.error('FAIL: mobile/tablet refresh controls below 44', { mobile, tablet });
  process.exit(1);
}
if (desktop.intervalH > 40) {
  console.error('FAIL: desktop refresh interval tall', desktop.intervalH);
  process.exit(1);
}
console.log('QA screenshots written to', outDir);
console.log('desktop', desktop.intervalH, desktop.toggleH, 'mobile', mobile.intervalH, mobile.toggleH, 'tablet', tablet.intervalH);
