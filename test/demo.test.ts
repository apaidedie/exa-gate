...
describe('demo ui script', () => {
  it('exposes a reproducible Chinese admin UI demo entrypoint', () => {
    ...
    expect(captureScript).toContain('#loginBoundaryNote');
    expect(captureScript).toContain('.tab-panel[data-tab-panel="overview"].active #dashHeroTitle');
    expect(captureScript).toContain('#dashHeroTitle');
    expect(captureScript).toContain('#trendRecap');
    expect(captureScript).toContain('#alertList');
    expect(captureScript).toContain('.dash-kpi-grid');
  ...
  it('keeps the README admin console preview reproducible and non-empty', () => {
    ...
    expect(readme).toContain('不是 Exa API Key');
    expect(readme).toContain('桌面运维总览');
    expect(readme).toContain('运行态势');
    expect(readme).toContain('用量趋势');
  ...
  it('documents the local demo console flow in Chinese', () => {
    ...
    expect(readme).toContain('生产向登录卡只接受管理员令牌');
    expect(readme).toContain('运行态势、健康密钥/请求/错误率 KPI、用量趋势、告警中心与密钥健康');
