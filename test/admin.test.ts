...
describe('admin api and ui', () => {
  ...
  it('serves a built-in admin web UI', async () => {
    ...
    expect(uiBundle).toContain('class="ops-card dash-ops-chart-card dash-side-health" aria-label="状态分布。可对照健康/冷却/禁用占比后打开密钥池筛选"');
    ...
    expect(uiBundle).toContain('class="failure-reasons" role="status" aria-live="polite" aria-atomic="true" aria-label="最近失败');
    expect(uiBundle).toContain('detail-hero');
    expect(uiBundle).toContain('detail-actions-primary');
    ...
    expect(uiBundle).toContain('class="ops-strip dash-ops-charts dash-ops-hidden"');
    ...
    expect(uiBundle).toContain('id="recentActivityRail" class="recent-activity-rail dash-ops-hidden" aria-label="最近请求活动。可点击条目打开日志或导入密钥"');
