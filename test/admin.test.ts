...
describe('admin api and ui', () => {
  ...
  it('serves a built-in admin web UI', async () => {
    ...
    const jsSource = [
      ...
      'src/admin-ui/logs/render-shared.js',
      'src/admin-ui/logs/render-list.js',
      'src/admin-ui/logs/render-trace.js',
      'src/admin-ui/audit/render.js',
      'src/admin-ui/keys/actions.js',
      'src/admin-ui/keys/import.js',
      'src/admin-ui/keys/ops.js',
      'src/admin-ui/keys/render-shared.js',
      'src/admin-ui/keys/render-summary.js',
      'src/admin-ui/keys/render-workflow.js',
    ...
    expect(uiBundle).toContain("setAuditStatus('auditFailure', fmt(failure), '审计失败', failure ? '可筛选失败记录并复核' : '当前无失败审计，可继续观察或刷新列表')");
    expect(uiBundle).toContain('<em aria-hidden="true">\' + esc(command.chip) + \'</em>');
    ...
    expect(uiBundle).toContain('id="configDetailHttps" class="config-item" tabindex="-1" data-config-posture-target="https" aria-label="登录保护：待同步。可点击配置证据中的 HTTPS 管理后在此复核"');
    expect(uiBundle).toContain('id="configDetailRawKey" class="config-item" tabindex="-1" data-config-posture-target="raw-key" aria-label="密钥安全：默认脱敏展示。可点击配置证据中的原始密钥后在此复核"');
    expect(uiBundle).toContain('id="configDetailPaths" class="config-item" tabindex="-1" data-config-posture-target="paths" aria-label="调度策略与路径：待同步。可点击配置证据中的路径策略后在此复核"');
    expect(uiBundle).toContain('id="configDetailState" class="config-item" tabindex="-1" data-config-posture-target="state" aria-label="状态存储：待同步。可点击配置证据中的状态存储后在此复核"');
    ...
    expect(uiBundle).toContain('aria-label="链路步骤路径：');
    expect(uiBundle).toContain('可点密钥打开详情，或回日志按路径筛选');
    expect(uiBundle).toContain('class="trace-chain" aria-label="密钥链路：\' + esc(summary.keyChain) + \'。可点密钥打开详情复核，或回日志按密钥筛选"');
    ...
    expect(uiBundle).toContain('aria-label="清空快速操作搜索，恢复全部命令。可继续输入关键词或用方向键选择执行"');
    ...
    expect(uiBundle).toContain('id="loginForm" class="login-card" aria-label="管理员登录表单。输入令牌后进入控制台"');
    ...
    expect(uiBundle).toContain('class="health-line" aria-label="健康密钥指标。可打开密钥池处理冷却或禁用项"');
    expect(uiBundle).toContain('class="state-stack" aria-label="状态占比条。可对照后打开密钥池筛选异常项"');
    expect(uiBundle).toContain('class="diag-list" aria-label="最近链路明细。可打开请求日志按错误或路径收窄"');
    expect(uiBundle).toContain('class="trend-body" aria-label="趋势主体。可点击摘要项调整窗口或筛选日志"');
    expect(uiBundle).toContain('class="toolbar" aria-label="密钥池工具栏。可搜索、筛选状态、测试当前页或批量导入"');
    expect(uiBundle).toContain('class="table-scroll key-table-scroll" aria-label="密钥表滚动区。可勾选、排序并打开详情"');
    expect(uiBundle).toContain('class="pager" aria-label="密钥分页。可调整每页数量或切换页码"');
    expect(uiBundle).toContain('class="pager-actions" aria-label="密钥分页操作。可调整每页数量、翻页或跳转"');
    expect(uiBundle).toContain('class="toolbar log-tools" aria-label="请求日志工具栏。可筛选、刷新列表、导出或清理过期日志"');
    expect(uiBundle).toContain('class="table-scroll log-table-scroll" aria-label="请求日志滚动区。可点 requestId 展开链路"');
    ...
    expect(uiBundle).toContain('class="insight-card good" id="insightJudgement" aria-label="当前判断卡片。可继续观察运行态势或刷新控制台"');
    expect(uiBundle).toContain('class="insight-card warn" id="insightNextAction" aria-label="下一步卡片。可点击执行建议动作"');
    expect(uiBundle).toContain('class="insight-card blue" id="insightWindow" aria-label="观测窗口卡片。可继续观察或点击调整趋势窗口"');
    ...
    expect(uiBundle).toContain('class="governance-card retention-governance-card" aria-label="日志治理卡片。可到配置详情复核保留窗口与过期日志"');
    expect(uiBundle).toContain('class="panel-head" aria-label="密钥池标题与工具。可搜索筛选或批量导入"');
    expect(uiBundle).toContain('class="panel-head" aria-label="请求日志标题与工具。可筛选、导出或清理过期日志"');
    expect(uiBundle).toContain('class="panel-head" aria-label="管理员审计标题与工具。可筛选动作/结果或导出审计"');
    expect(uiBundle).toContain('class="panel-head" aria-label="运行配置标题。可点击证据项查看配置详情"');
    expect(uiBundle).toContain('class="modal-head" aria-label="批量导入标题。可关闭返回密钥池"');
    expect(uiBundle).toContain('class="modal-body" aria-label="批量导入内容。可粘贴密钥或选择文件后预检"');
    expect(uiBundle).toContain('class="modal-foot" aria-label="批量导入操作。可取消返回或开始导入"');
    expect(uiBundle).toContain('class="modal-head" aria-label="确认对话框标题。可关闭返回控制台"');
    expect(uiBundle).toContain('class="modal-body" aria-label="确认说明区。可阅读后确认执行或取消返回"');
    expect(uiBundle).toContain('class="modal-foot" aria-label="确认操作按钮。可确认执行或取消返回"');
    ...
    expect(uiBundle).toContain("关闭快速操作，返回控制台。可继续管理密钥或刷新状态");
    ...
    expect(uiBundle).toContain('id="cancelImport" class="ghost-btn" type="button" aria-label="取消批量导入，返回密钥池。可继续筛选或管理密钥"');
    expect(uiBundle).toContain('id="confirmImport" class="primary-btn" type="button" aria-label="开始导入不可用。请先粘贴或选择可导入密钥" disabled');
    expect(uiBundle).toContain('id="closeImportModal" class="modal-close" type="button" aria-label="关闭批量导入，返回密钥池。可继续筛选或管理密钥"');
    ...
    expect(uiBundle).toContain('id="confirmActionCancel" class="ghost-btn" type="button" aria-label="取消确认操作，返回控制台。可继续当前运维操作"');
    ...
    expect(uiBundle).toContain('id="closeConfirmAction" class="modal-close" type="button" aria-label="关闭确认对话框，返回控制台。可继续当前运维操作"');
    ...
    expect(uiBundle).toContain('id="closeMobileDetails" class="ghost-btn" type="button" aria-label="关闭移动端密钥详情，返回密钥表。可继续点选密钥或批量操作"');
    expect(uiBundle).toContain('function closeMobileDetailsPanel');
    expect(uiBundle).toContain("state.mobileDetailsOpen = false");
    expect(uiBundle).toContain('移动端密钥详情已打开。可复核用量与操作，或关闭返回密钥表');
    expect(uiBundle).toContain('关闭移动端密钥详情，返回密钥表。可继续点选密钥或批量操作');
    expect(uiBundle).toContain('关闭移动端密钥详情。可点选密钥后打开详情');
    expect(uiBundle).toContain('可继续测试、查看日志或调整启用状态');
