...
test('admin console covers login, key actions, logs export, and webhook testing', async ({ page }) => {
  ...
  await expect(page.locator('#sidebarCollapse')).toHaveAttribute('aria-label', '收起侧栏导航');
  ...
  await expect(page.locator('#sidebarCollapse')).toHaveAttribute('aria-label', '展开侧栏导航');
  await page.click('#sidebarCollapse');
