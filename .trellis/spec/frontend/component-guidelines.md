...
- In flex-column tab panels, new standalone overview/status sections that are not `.panel` children also need `flex: 0 0 auto`. Otherwise the section box can collapse while its internal buttons visually overflow into the next grid, causing `elementFromPoint()` and real clicks to hit the following section instead of the visible control.
